...
properties = ['diameter', 'calcification', 'malignancy', 'spiculation', 'texture']
classes = {
...
property_weights = {'diameter': 0.7, 'malignancy': 1.5}
...
def build_objective(model, deterministic=False, epsilon=1e-12):
    ...
    for i, p in enumerate(properties):
        ...
        if p in property_weights:
            losses[-1] *= property_weights[p]
    loss = T.sum(losses)
    return loss, losses
