...
malignancy_bins = [1, 2, 3, 4, 5]
n_malignant_classes = len(malignancy_bins)
malignancy_priors = [1. / 6] * 6
...
def label_prep_fun(label):
    if label == 0:
        ...
        return np.digitize(label, malignancy_bins)
...
def build_model(l_in=None):
    ...
    softmax_bias = np.log(malignancy_priors)
    l_out = nn.layers.DenseLayer(l, num_units=n_malignant_classes,
