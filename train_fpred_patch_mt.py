...
for chunk_idx, (xs_chunk_train, y_chunk_train, id_train) in izip(chunk_idxs, buffering.buffered_gen_threaded(
    ...
    if ((chunk_idx + 1) % config().validate_every) == 0:
        ...
        tmp_losses_valid = []
        for i, (xs_chunk_valid, y_chunk_valid, ids_batch) in enumerate(
                buffering.buffered_gen_threaded(valid_data_iterator.generate(),
                                                buffer_size=2)):
            for x_shared, x in zip(xs_shared, xs_chunk_valid):
                x_shared.set_value(x)
