...
weight_decay_schedule = config().weight_decay_schedule
weight_decay =theano.shared(np.float32(weight_decay_schedule[0]))
reg_norm=config().reg_norm

for index,param in enumerate(params):
    if str(param) is 'b':
        wd_scale=theano.tensor.constant(0.0) 
    else:
        wd_scale=theano.tensor.constant(1.0)
    
    train_loss += wd_scale * reg_norm(param) * weight_decay 
...
iter_test = theano.function([], valid_loss, givens=givens_valid)
...
if config().restart_from_save:
    ...
    losses_eval_test = resume_metadata['losses_eval_test']
else:
...
for chunk_idx, (x_chunk_train, y_chunk_train, id_train) in izip(chunk_idxs, buffering.buffered_gen_threaded(
    ...
    if chunk_idx in weight_decay_schedule:
        wd = np.float32(weight_decay_schedule[chunk_idx])
        print '  setting weight decay to %.7f' % wd
        print
        learning_rate.set_value(wd)
    ...
    if ((chunk_idx + 1) % config().validate_every) == 0:
        ...
        for i, (x_chunk_test, y_chunk_test, ids_batch) in enumerate(
                buffering.buffered_gen_threaded(test_data_iterator.generate(),
                                                buffer_size=2)):
            x_shared.set_value(x_chunk_test)
            y_shared.set_value(y_chunk_test)
       
            l_test = iter_test()
            print i, l_test, y_chunk_test, ids_batch
            tmp_losses_test.append(l_test)

        # calculate validation loss across validation set
        test_loss = np.mean(tmp_losses_test)
        print 'Test loss: ', test_loss
        losses_eval_test.append(test_loss)
    ...
    if ((chunk_idx + 1) % config().save_every) == 0:
        ...
        with open(metadata_path, 'w') as f:
            pickle.dump({
                ...
                'losses_eval_test': losses_eval_test,
