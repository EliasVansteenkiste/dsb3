...
OUTPUT_FOLDER = "/media/lio/Elements/dsb3/prep/"
...
for i, patient in enumerate(patients):
    scan, same0 = preptools.load_scan(INPUT_FOLDER_STAGE1 + patient)
    print "%i/%i" % (i + 1, len(patients)), patient  # , pixels.shape
    # if scan is None: continue
    # else: print patient
    # scan = preptools.load_scan(INPUT_FOLDER_STAGE1 + patient, stop_before_pixels=True)
    pixels, same = preptools.get_pixels_hu(scan)
    if same and same0: continue
    else: print patient
