...
def in_mask(roi, ct_scan):
    ...
    d0_segmentation_mask = get_segmentation_mask(ct_scan[roi[0],:,:], z=1,plot=False,savefig=False)
    d1_segmentation_mask = get_segmentation_mask(ct_scan[:,roi[1],:], z=2,plot=False,savefig=False)
    d2_segmentation_mask = get_segmentation_mask(ct_scan[:,:,roi[2]], z=3,plot=False,savefig=False)
...
def check_nodules(set, plot=False):
    ...
    for _i, sample_id in enumerate(set_indices):
        ...
        #normalized_vol = normHU2HU(volume)
        rois_in_mask = []
        for roi in rois:
