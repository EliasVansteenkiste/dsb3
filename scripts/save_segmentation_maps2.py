...
# Resume
new_patient_paths = []
for patient_path in data_iterator.patient_paths:
    pid = utils_lung.extract_pid_dir(patient_path)
    if not os.path.exists(dst_path + '/%s.pkl' % pid):
        new_patient_paths.append(patient_path)
    else:
        print pid, "exists"
data_iterator.patient_paths = new_patient_paths
...
for n, (x, lung_mask, tf_matrix, pid) in enumerate(
    ...
    print '-------------------------------------'
    print n, pid
    if os.path.exists(dst_path + '/%s.pkl' % pid):
        print "exists"
        continue

    predictions_scan = np.zeros((1, n_windows * stride, n_windows * stride, n_windows * stride))
