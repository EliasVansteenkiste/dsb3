...
def build_model():
    ...
    net['contr_1_3'] = conv3d(l_in, base_n_filters)
    net['contr_1_3'] = nn.layers.ParametricRectifierLayer(net['contr_1_3'])
    net['contr_1_4'] = conv3d(net['contr_1_3'], base_n_filters)
    net['contr_1_4'] = nn.layers.ParametricRectifierLayer(net['contr_1_4'])

    net['concat1_1'] = nn.layers.ConcatLayer([net['contr_1_4'], net['contr_1_2']])

    net['pool1'] = max_pool3d(net['concat1_1'])

    net['encode_1'] = conv3d(net['pool1'], base_n_filters * 2)
    ...
    net['encode_3'] = conv3d(net['encode_2'], base_n_filters * 2)
    ...
    net['expand_1_1'] = conv3d(net['concat1'], num_filters=2 * base_n_filters)
    net['expand_1_1'] = nn.layers.ParametricRectifierLayer(net['expand_1_1'])
    net['expand_1_2'] = conv3d(net['expand_1_1'], num_filters=2 * base_n_filters)
    net['expand_1_2'] = nn.layers.ParametricRectifierLayer(net['expand_1_2'])
    net['expand_1_3'] = conv3d(net['expand_1_2'], num_filters=2 * base_n_filters)
...
def build_objective(model, deterministic=False, epsilon=1e-12):
    ...
    dice = (2. * T.sum(targets * predictions)) / (T.sum(predictions) + T.sum(targets))
    return -1. * dice
