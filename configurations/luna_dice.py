...
def build_objective_ce(model, deterministic=False, epsilon=1e-15):
...
def build_objective(model, deterministic=False, epsilon=1e-15):
    predictions = T.flatten(nn.layers.get_output(model.l_out, deterministic=deterministic),outdim=2)
    targets = T.flatten(nn.layers.get_output(model.l_target),outdim=2)
    intersection = T.sum(predictions * targets)
    dice_coeff=(2. * intersection + epsilon) / (T.sum(targets) + T.sum(predictions) +epsilon)
