...
order_objectives = ['malignancy'] 


property_type = {'malignancy': 'bounded_continuous'}


property_bin_borders = {}


norm_weights_loss = {'malignancy': 1.} #because it is important

init_values_final_units = {'malignancy': .1}
...
train_data_iterator = data_iterators.CandidatesLunaPropsDataGenerator(data_path=pathfinder.LUNA_DATA_PATH,
								...
								property_type = property_type,
                                                                 return_enable_target_vector = False)
...
def bce(target_idx, prediction_idx, predictions, targets):
...
def build_objective(model, deterministic=False, epsilon=1e-12):
    ...
    for obj_idx, obj_name in enumerate(order_objectives):
        ...
        if ptype == 'classification':
            ...
            v_obj = bce(obj_idx, unit_ptr, predictions, targets)
            obj_scalar =  T.mean(v_obj)
            unit_ptr += 1
        else:
            raise
