...
def data_prep_function(data, patch_center, pixel_spacing, luna_origin, p_transform,
    ...
    bins, original_borders = data_transforms.get_rescale_params_hist_eq(data)
...
order_objectives = ['size', 
                    'spiculation', 
                    'malignancy', 
                    'texture'] 


property_type = {'size': 'continuous',
            'spiculation': 'bounded_continuous',
            'malignancy': 'bounded_continuous',
            'texture': 'classification'}   # 0:nothing, 1: non-solid/ground glass, 2: mixed, 3: solid


property_bin_borders = {'texture': [1.5,2.5,3.5,4.5,100]}


norm_weights_loss = {'size': 1./30.,
            'spiculation': 1.,
            'malignancy': 1.,
            'texture': 1./3}  #class
...
def bounded_continuous_crossentropy(target_idx, prediction_idx, predictions, targets, epsilon):
    ...
    crossentropy = targets * T.log(predictions) + (1-targets)*T.log(1.-predictions)
    bounded_continuous_ce = crossentropy - (targets * T.log(targets) + (1-targets)*T.log(1.-targets))
    return bounded_continuous_ce
...
def build_objective(model, deterministic=False, epsilon=1e-12):
    ...
    for obj_idx, obj_name in enumerate(order_objectives):
        ...
        if ptype == 'classification':
            ...
            #v_obj = bounded_continuous_crossentropy(obj_idx, unit_ptr, predictions, targets, epsilon)
            v_obj = sqe(obj_idx, unit_ptr, predictions, targets)
            obj_scalar =  T.mean(v_obj)
            unit_ptr += 1
        else:
