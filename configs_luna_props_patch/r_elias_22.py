...
def build_model():
    ...
    l = inrn_v2_red(l)
    l_feat = inrn_v2_red(l)

    final_layers = []
    unit_ptr = 0
    for obj_idx, obj_name in enumerate(order_objectives):
        l = dense(drop(l_feat), 64, name='dense_'+obj_name)
        ...
        if ptype == 'classification':
            ...
            l_fin = nn.layers.DenseLayer(l, num_units=num_units,
                     ...
                     nonlinearity=nn.nonlinearities.softmax, name='final_'+ptype+'_'+obj_name)

        elif ptype == 'continuous':
            l_fin = nn.layers.DenseLayer(l, num_units=1,
                    W=lasagne.init.Orthogonal(),
                    b=lasagne.init.Constant(init_values_final_units[obj_name]),
                    nonlinearity=nn.nonlinearities.softplus, name='final_'+ptype+'_'+obj_name)
