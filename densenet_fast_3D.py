...
from lasagne.layers.dnn import Pool3DDNNLayer as Pool3DLayer
...
def transition(network, dropout, name_prefix):
    ...
    network = Pool3DLayer(network, 2, mode='average_inc_pad',
...
def affine_relu_conv(network, channels, filter_size, dropout, name_prefix):
    ...
    network = Conv3DLayer(network, channels, filter_size, pad='same',
                          W=lasagne.init.HeNormal(gain='relu'),
