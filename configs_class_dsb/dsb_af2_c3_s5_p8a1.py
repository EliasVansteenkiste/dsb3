...
def build_model():
    ...
    network = load_pretrained_model(l_in_rshp)

    l_out = nn.layers.DenseLayer(network, num_units=1,
                                 W=nn.init.Constant(0.),
                                 b=nn.init.Constant(0.),
                                 nonlinearity=None)

    l = nn.layers.ReshapeLayer(l_out, (-1, n_candidates_per_patient, 1))

    l_out = nn_lung.AggAllBenignWeightedExp(l,n_candidates_per_patient)
