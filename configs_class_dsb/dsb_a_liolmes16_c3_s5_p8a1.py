...
def build_model():
    ...
    l_out = nn_lung.LogMeanExpScaled(l, r=16, n_cand=n_candidates_per_patient, axis=(1, 2), name='LME')
...
def build_objective(model, deterministic=False, epsilon=1e-12):
    ...
    loss = T.mean(bce)
    if not deterministic: return loss #+ 1e-4*nn.regularization.regularize_layer_params(model.l_out, nn.regularization.l2)
    else: return loss
