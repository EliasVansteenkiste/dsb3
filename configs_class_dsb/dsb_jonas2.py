...
def build_nodule_classification_model(l_in):
    ...
    # p = the correct prior odds that a single nodule is cancerous?

    p = 1 - (1-0.26) ** (1. / n_candidates_per_patient)  # every cancerous patient has 1 cancerous nodule
    l_out = nn.layers.DenseLayer(model.l_out.input_layer,
                                 num_units=1,
                                 W=nn.init.Constant(0.),
                                 b=nn.init.Constant(np.log((1.-p)/p)),
