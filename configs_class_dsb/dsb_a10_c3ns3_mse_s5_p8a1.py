...
learning_rate_schedule = {
    0: 1e-4,
    int(0.5 * max_nchunks): 2e-5,
    int(0.6 * max_nchunks): 1e-5,
    int(0.7 * max_nchunks): 5e-5,
    int(0.9 * max_nchunks): 2e-6
}
...
def build_nodule_size_model(l_in):
    ...
    # remove convolutional layers from training
    nn_lung.remove_trainable_parameters(model.l_out.input_layer.input_layer)
    return model
...
def build_model():
    ...
    l_roi.input_layer.W.tag.grad_scale = 0.05
    l_roi.input_layer.b.tag.grad_scale = 0.05
