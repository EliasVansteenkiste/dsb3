...
def load_pretrained_model(l_in):
    ...
    l = nn.layers.DenseLayer(l,1,nonlinearity=None, W=nn.init.Orthogonal(),
                b=nn.init.Constant(0))

    l = nn.layers.NonlinearityLayer(l,nonlinearity=nn.nonlinearities.sigmoid)
...
def build_model():
    ...
    l_coords = nn.layers.DenseLayer(l_in_coords_rshp,50,W=nn.init.Orthogonal(),nonlinearity=nn.nonlinearities.very_leaky_rectify)
    l_coords = nn.layers.DenseLayer(l_coords, 50, W=nn.init.Orthogonal(),nonlinearity=nn.nonlinearities.very_leaky_rectify)
    l_coords = nn.layers.DenseLayer(l_coords,1,nonlinearity=None, W=nn.init.Orthogonal(),
                b=nn.init.Constant(0))
    ...
    nn.layers.set_all_param_values(l_coords, metadata['param_values'])

    l = load_pretrained_model(l_in_rshp)
    all_layers = nn.layers.get_all_layers(l)
    l_sum = nn.layers.ElemwiseSumLayer([l_coords,all_layers[-2]])

    all_layers[-1].input_layer=l_sum
