...
if type_set == 'test':
    ...
    pid2prediction = {}
    for i, (x_test,x_test_coords, _, id_test) in enumerate(buffering.buffered_gen_threaded(
            data_iterator.generate())):
        predictions = iter_test(x_test,x_test_coords)
        pid = id_test[0]
    ...
    for i, (x_test,x_test_coords, y_test, id_test) in enumerate(buffering.buffered_gen_threaded(
            data_iterator.generate())):
        predictions = iter_test(x_test,x_test_coords)
        pid = id_test[0]
        pid2prediction[pid] = predictions[0, 1] if predictions.shape[-1] == 2 else predictions[0]
