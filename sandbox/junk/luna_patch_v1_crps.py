...
def build_model():
    ...
    # Z
    l_dz = nn.layers.DenseLayer(l, num_units=128, W=nn.init.Orthogonal("relu"),
                                b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.very_leaky_rectify)
    mu_z = nn.layers.DenseLayer(nn.layers.dropout(l_dz, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                b=nn.init.Constant(patch_size[0] / 2), nonlinearity=nn.nonlinearities.rectify)
    sigma_z = nn.layers.DenseLayer(nn.layers.dropout(l_dz, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                   b=nn.init.Constant(1), nonlinearity=nn.nonlinearities.rectify)
    lz = nn_lung.NormalCDFLayer(mu_z, sigma_z, patch_size[0])

    # Y
    l_dy = nn.layers.DenseLayer(l, num_units=128, W=nn.init.Orthogonal("relu"),
                                b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.very_leaky_rectify)
    mu_y = nn.layers.DenseLayer(nn.layers.dropout(l_dy, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                b=nn.init.Constant(patch_size[1] / 2), nonlinearity=nn.nonlinearities.rectify)
    sigma_y = nn.layers.DenseLayer(nn.layers.dropout(l_dy, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                   b=nn.init.Constant(1), nonlinearity=nn.nonlinearities.rectify)
    ly = nn_lung.NormalCDFLayer(mu_y, sigma_y, patch_size[1])

    # X
    l_dx = nn.layers.DenseLayer(l, num_units=128, W=nn.init.Orthogonal("relu"),
                                b=nn.init.Constant(0.1), nonlinearity=nn.nonlinearities.very_leaky_rectify)
    mu_x = nn.layers.DenseLayer(nn.layers.dropout(l_dx, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                b=nn.init.Constant(patch_size[2] / 2), nonlinearity=nn.nonlinearities.rectify)
    sigma_x = nn.layers.DenseLayer(nn.layers.dropout(l_dx, p=0.5), num_units=1, W=nn.init.Orthogonal(),
                                   b=nn.init.Constant(1), nonlinearity=nn.nonlinearities.rectify)
    lx = nn_lung.NormalCDFLayer(mu_x, sigma_x, patch_size[2])

    l_out = nn.layers.ConcatLayer([lz, ly, lx])

    return namedtuple('Model', ['l_in', 'l_out', 'l_target', 'lz', 'ly', 'lx'])(l_in, l_out, l_target,
                                                                                lz, ly, lx)
